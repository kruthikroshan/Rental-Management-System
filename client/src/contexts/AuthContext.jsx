...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (data.requiresVerification) {
        return {
          ...
          otp: data.otp  // Include OTP from backend response
        };
      } else {
        setAccessToken(data.accessToken);
        dispatch({ type: 'SET_USER', payload: data.user });
  ...
  const requestPhoneOTP = async (phoneNumber) => {
    try {
      ...
      return { success: true, message: data.message, otp: data.otp, isNewUser: data.isNewUser };
    } catch (error) {
      const message = error.response?.data?.message || 'Failed to request phone OTP';
      dispatch({ type: 'SET_ERROR', payload: message });
      return { success: false, error: message };
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
  ...
  const verifyPhoneOTP = async (verifyData) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      dispatch({ type: 'SET_ERROR', payload: null });
      const data = await authAPI.verifyPhoneOTP(verifyData);
      setAccessToken(data.accessToken);
      dispatch({ type: 'SET_USER', payload: data.user });
      return { success: true, message: data.message };
    } catch (error) {
